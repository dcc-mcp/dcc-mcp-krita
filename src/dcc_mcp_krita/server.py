"""DCC-MCP composition root for the external Krita host bridge."""
...
SERVER_NAME = "dcc-mcp-krita"
_BUILTIN_SKILLS_DIR = Path(__file__).resolve().parent / "skills"
_server: Optional["KritaMcpServer"] = None


class KritaMcpServer(DccServerBase):
    """Sidecar adapter using an authenticated, main-thread Krita plug-in bridge."""
    ...
    def register_builtin_actions(
        ...
        if minimal_mode is None:
            minimal_mode = MinimalModeConfig(skills=("krita-document-authoring",))
...
def start_server(
    ...
    **kwargs: Any,
    ...
    if _server is None:
        ...
        server.start()
        _server = server
        logger.info("Krita MCP server started")
    return _server
