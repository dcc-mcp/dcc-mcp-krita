"""Exercise the real KRITA plug-in bridge and MCP tools."""
...
def main() -> None:
    ...
    while time.monotonic() < deadline:
        ...
        raise RuntimeError("KRITA plug-in bridge did not become ready")

    server = KritaMcpServer(port=0)
    try:
        server.register_builtin_actions()
        server.start(install_atexit_hook=False)
        url = server.mcp_url
        call(url, "load_skill", {"skill_name": "krita-session"})
